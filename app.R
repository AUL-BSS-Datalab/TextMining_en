...
#Indlæs den tyske stopordsliste fra Stop_Words mappen
stop_words_de <- read.csv("Stopwords/stopwords_de.csv")
#Indlæs den franske stopordsliste fra Stop_Words mappen
stop_words_fr <- read.csv("Stopwords/stopwords_fr.csv")
#Indlæs den spanske stopordsliste fra Stop_Words mappen
stop_words_es <- read.csv("Stopwords/stopwords_es.csv")
#Indlæs den japanske stopordsliste fra Stop_Words mappen
stop_words_ja <- read.csv("Stopwords/stopwords_ja.csv")
#Indlæs den kinesiske stopordsliste fra Stop_Words mappen
stop_words_zh <- read.csv("Stopwords/stopwords_zh.csv")
#Indlæs den russiske stopordsliste fra Stop_Words mappen
stop_words_ru <- read.csv("Stopwords/stopwords_ru.csv")
...
#-------------------------- Indlæs og klargøring filer --------------------------------------------------------  
  ...
  #Tidy version uden stopord (tysk)
  tidy_corpus_de <- reactive({
    req(input$files)
    #Læs teksten fra filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
  ...
  #Tidy version uden stopord (fransk)
  tidy_corpus_fr <- reactive({
    req(input$files)
    #Læs teksten fra filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
  ...
  #Tidy version uden stopord (spansk)
  tidy_corpus_es <- reactive({
    req(input$files)
    #Læs teksten fra filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
  ...
  #Tidy version uden stopord (japansk)
  tidy_corpus_ja <- reactive({
    req(input$files)
    #Læs teksten fra filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
    ...
    raw_corpus %>% 
      unnest_tokens(word, text) %>% 
      anti_join(stop_words_ja)
  })
  #Tidy version uden stopord (kinesisk)
  tidy_corpus_zh <- reactive({
    req(input$files)
    #Læs teksten fra filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
    ...
    raw_corpus %>% 
      unnest_tokens(word, text) %>% 
      anti_join(stop_words_zh)
  })
  
  #Tidy version uden stopord (russisk)
  tidy_corpus_ru <- reactive({
    req(input$files)
    #Læs teksten fra filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
  ...
  #Forbered tidy version af bigrams (tysk)
  tidy_bigram_de <- reactive({
    req(input$files)
    #Læs teksten fra txt filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
    ...
    raw_corpus %>% 
      ...
      filter(!is.na(bigram)) %>% 
      separate(bigram, c("word1", "word2"), sep = " ") %>% 
      filter(!word1 %in% stop_words_de$word) %>% 
      filter(!word2 %in% stop_words_de$word)
  }) 
  
  #Forbered tidy version af bigrams (fransk)
  tidy_bigram_fr <- reactive({
    req(input$files)
    #Læs teksten fra txt filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
  ...
  #Forbered tidy version af bigrams (spansk)
  tidy_bigram_es <- reactive({
    req(input$files)
    #Læs teksten fra txt filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
  ...
  #Forbered tidy version af bigrams (japansk)
  tidy_bigram_ja <- reactive({
    req(input$files)
    #Læs teksten fra txt filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
    ...
    raw_corpus %>% 
      ...
      filter(!is.na(bigram)) %>% 
      separate(bigram, c("word1", "word2"), sep = " ") %>% 
      filter(!word1 %in% stop_words_ja$word) %>% 
      filter(!word2 %in% stop_words_ja$word)
  }) 
  
  #Forbered tidy version af bigrams (kinnesisk)
  tidy_bigram_zh <- reactive({
    req(input$files)
    #Læs teksten fra txt filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
    ...
    raw_corpus %>% 
      ...
      filter(!is.na(bigram)) %>% 
      separate(bigram, c("word1", "word2"), sep = " ") %>% 
      filter(!word1 %in% stop_words_zh$word) %>% 
      filter(!word2 %in% stop_words_zh$word)
  }) 
  #Forbered tidy version af bigrams (russisk)
  tidy_bigram_ru <- reactive({
    req(input$files)
    #Læs teksten fra txt filerne
    tibbles_of_texts <- map_df(input$files$datapath, readtext) %>% 
      mutate(title = str_remove(input$files$name, ".txt")) %>% 
    ...
    raw_corpus %>% 
      ...
      filter(!is.na(bigram)) %>% 
      separate(bigram, c("word1", "word2"), sep = " ") %>% 
      filter(!word1 %in% stop_words_ru$word) %>% 
      filter(!word2 %in% stop_words_ru$word)
  }) 
...
#---------------------------- Nærlæs_tekst ----------------------------------------------------
  ...
  output$viz_text <- renderText({
    ...
    } else if (input$selected_stopword_view == "Without" & input$language_stopwords == "de"){
      selected_text_data_read <- tidy_corpus_de()
      
    } else if (input$selected_stopword_view == "Without" & input$language_stopwords == "fr"){
      selected_text_data_read <- tidy_corpus_fr()
      
    }else if (input$selected_stopword_view == "Without" & input$language_stopwords == "es"){
      selected_text_data_read <- tidy_corpus_es()
      
    }else if (input$selected_stopword_view == "Without" & input$language_stopwords == "ja"){
      selected_text_data_read <- tidy_corpus_ja()
      
    }else if (input$selected_stopword_view == "Without" & input$language_stopwords == "zh"){
      selected_text_data_read <- tidy_corpus_zh()
      
    } else if (input$selected_stopword_view == "Without" & input$language_stopwords == "ru"){
      selected_text_data_read <- tidy_corpus_ru()
    }
...
#------------------------ Søjlediagram --------------------------------------------------
  ...
  output$viz_plot <- renderPlot({
    if (input$language_stopwords == "en"){
      ...
      }else if(input$language_stopwords == "de"){
        selected_text_data_plot <- tidy_corpus_de()
      }else if(input$language_stopwords == "fr"){
        selected_text_data_plot <- tidy_corpus_fr()
      }else if(input$language_stopwords == "es"){
        selected_text_data_plot <- tidy_corpus_es()
      }else if(input$language_stopwords == "ja"){
        selected_text_data_plot <- tidy_corpus_ja()
      }else if(input$language_stopwords == "zh"){
        selected_text_data_plot <- tidy_corpus_zh()
      }else if(input$language_stopwords == "ru"){
        selected_text_data_plot <- tidy_corpus_ru()
      }
...
output$viz_wordcloud <- renderPlot({
  ...
  if (input$language_stopwords == "en"){
    ...
    selected_text_data_cloud <- tidy_corpus_de()
  }else if(input$language_stopwords == "fr"){
    selected_text_data_cloud <- tidy_corpus_fr()
  }else if(input$language_stopwords == "es"){
    selected_text_data_cloud <- tidy_corpus_es()
  }else if(input$language_stopwords == "ja"){
    selected_text_data_cloud <- tidy_corpus_ja()
  }else if(input$language_stopwords == "zh"){
    selected_text_data_cloud <- tidy_corpus_zh()
  }else if(input$language_stopwords == "ru"){
    selected_text_data_cloud <- tidy_corpus_ru()
  }
...
#------------------------- Bigrams --------------------------------------------------------
  ...
  output$viz_bigrams <- renderPlot({
    ...
    if (input$language_stopwords == "en"){
      ...
      selected_text_data_bigrams <- tidy_bigram_de()
    }else if(input$language_stopwords == "fr"){
      selected_text_data_bigrams <- tidy_bigram_fr()
    }else if(input$language_stopwords == "es"){
      selected_text_data_bigrams <- tidy_bigram_es()
    }else if(input$language_stopwords == "ja"){
      selected_text_data_bigrams <- tidy_bigram_ja()
    }else if(input$language_stopwords == "zh"){
      selected_text_data_bigrams <- tidy_bigram_zh()
    }else if(input$language_stopwords == "ru"){
      selected_text_data_bigrams <- tidy_bigram_ru()
    }
...
#--------------------------- Begrebsafklaring ------------------------------------------------
  ...
  output$term_info_text_3 <- renderText({
    paste("The stop word lists contain, respectively, English, Danish, German, French, Spanish, Japanese, Chinese and Russian stop words. The integrated stop word lists for comes from the R package stopwords and includes words from the three lexicons: onix, SMART, and snowball. The stop word list for Danish texts has been compiled by information specialists at the Royal Library. Both stop word lists can be found in the Stopwords folder.")
  })
